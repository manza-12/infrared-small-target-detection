...
class ASKCResNetFPN(nn.Module):
    def __init__(self, layer_blocks, channels, fuse_mode='AsymBi'):
        ...
        # NOTE: no BatchNorm on input image — the input is already
        # normalized by transforms.Normalize; a stem BN would
        # re-shift the distribution and cause train/eval mismatch.
...
class ASKCResUNet(nn.Module):
    def __init__(self, layer_blocks, channels, fuse_mode='AsymBi'):
        ...
        # NOTE: no BatchNorm on input image — see ASKCResNetFPN comment.
        self.stem = nn.Sequential(
            nn.Conv2d(3, stem_width, 3, 2, 1, bias=False),
            nn.BatchNorm2d(stem_width),
            nn.ReLU(True),
