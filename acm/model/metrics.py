...
class SamplewiseSigmoidMetric():
    ...
    def batch_intersection_union(self, output, target, nclass, score_thresh):
        """mIoU"""
        # Binary infrared small-target masks: compute scalar inter/union per image.
        predict = (F.sigmoid(output).detach().cpu().numpy() > score_thresh) # P
        target = target.detach().cpu().numpy() > 0 # T

        num_sample = predict.shape[0]
        ...
        for b in range(num_sample):
            pred_b = np.squeeze(predict[b]).astype(bool)
            label_b = np.squeeze(target[b]).astype(bool)
            if pred_b.shape != label_b.shape:
                raise ValueError(
                    'Prediction and label shapes do not match after squeeze: '
                    f'{pred_b.shape} vs {label_b.shape}'
                )

            area_inter = np.logical_and(pred_b, label_b).sum()
            area_inter_arr[b] = area_inter

            area_pred = pred_b.sum()
            area_pred_arr[b] = area_pred

            area_lab = label_b.sum()
            area_lab_arr[b] = area_lab

            area_union = np.logical_or(pred_b, label_b).sum()
